...
void bfinject_rocknroll() {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSLog(@"[bfdecryptor] excutablePath: %@", excutablePath);
...
%ctor {
    @autoreleasepool {
        ...
        NSMutableDictionary *prefs = [[NSMutableDictionary alloc] initWithContentsOfFile:ROOT_PATH_NS(@"/var/mobile/Library/Preferences/alias20.bfdecryptor.plist")];
        
        if([prefs[bundleID] boolValue])
        {
            NSLog(@"[bfdecryptor] Successfully Loaded");
            bfinject_rocknroll();
        }
    }
}
